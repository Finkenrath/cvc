 ...
 * vp_disc_hpe_stoch.c
 *
 * Tue Jan 26 13:03:00 CET 2010 
 *
 * PURPOSE:
 * - calculate the stochastic propagator part of the disconnected contractions 
 *   of the vacuum polarization; combine up- and down-part
 * TODO:
 * DONE:
 * CHANGES:
 *********************************************************************************/
  ...
  for(gid=g_gaugeid; gid<=g_gaugeid2; gid++) {
    ...
    for(sid=g_sourceid; sid<=g_sourceid2; sid+=g_sourceid_step) {
      ...
      if(count%Nsave == 0) {
        ...
        sprintf(filename, "vp_disc_ud_X.%.4d.%.4d", gid, count);
        sprintf(contype, "cvc-disc-u_and_d-stoch-X");
        write_lime_contraction(work, filename, 64, 4, contype, gid, count);
        sprintf(filename, "vp_disc_ud_X.%.4d.%.4d.ascii", gid, count);
        ...
        sprintf(filename, "vp_disc_ud_P.%.4d.%.4d", gid, count);
        sprintf(contype, "cvc-disc-u_and_d-stoch-P");
        write_lime_contraction(work+_GWI(8,0,VOLUME), filename, 64, 16, contype, gid, count);
        sprintf(filename, "vp_disc_ud_P.%.4d.%.4d.ascii", gid, count);
