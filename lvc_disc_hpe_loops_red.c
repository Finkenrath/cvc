...
#  endif
  ...
  /*****************************
   * initialize the HPE fields
   *****************************/
  init_hpe_fields(loop_tab, sigma_tab, shift_start, tcf, (int**)NULL);
    ...
    for(mu=0; mu<4; mu++) {
...
#endif
        init_lvc_trace_coeff_red(&(tcf[deg]), &(loop_tab[deg]), 
          &(sigma_tab[deg]), &(shift_start[deg]), deg, nloop+deg, mu);
        if(g_cart_id==0) fprintf(stdout, "# start reducing loops\n");
        reduce_loop_tab(loop_tab[deg], sigma_tab[deg], shift_start[deg], deg-1, nloop[deg]);
#ifdef MPI
        retime = MPI_Wtime();
#else
        retime = (double)clock() / CLOCKS_PER_SEC;
#endif
        if(g_cart_id==0) fprintf(stdout, "# time to initialize trace coefficients of order %d: %e seconds\n", deg, retime-ratime);
      }
      ...
      for(deg=hpe_order_min; deg<=hpe_order_max; deg+=2) {
        ...
        for(ix=0; ix<VOLUME; ix++) { 
          Hopping_lvc_iter_red( disc+_GWI(mu,ix,VOLUME), tcf[deg], ix, \
            mu, deg, nloop[deg], loop_tab[deg], sigma_tab[deg], shift_start[deg]);
        }
        ...
        retime = (double)clock() / CLOCKS_PER_SEC;
#endif
        if(g_cart_id==0) fprintf(stdout, "# time to calculate contribution from deg %d for gauge id %d: %e seconds\n", deg, gid, retime-ratime);
      }

    }

/*
    sprintf(filename, "lvc_disc_hpe%.2d_loops_Xascii.%.4d", hpe_order_max, gid);
    ...
    fclose(ofs);
*/
    sprintf(filename, "lvc_disc_hpe%.2d_loops_X.%.4d", hpe_order_max, gid);
    sprintf(contype, "lvc-disc-loops-hpe-%.2d-to-%.2d-X", hpe_order_min, hpe_order_max);
...
*/
   ...
   * free the HPE fields
   ******************************************************************************************/
  free_hpe_fields(loop_tab, sigma_tab, shift_start, tcf, (int**)NULL);
