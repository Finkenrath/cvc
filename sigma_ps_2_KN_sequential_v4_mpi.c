/****************************************************
 * sigma_ps_2_KN_sequential_v4_mpi.c
 *
 * Sun Jul 28 18:23:34 EEST 2013
 *
 * PURPOSE
 * - originally copied from sigma_ps_2_pi_lambda_sequential_v4_mpi.c
 * - contractions for 3-point function <Sigma^{*+} Kbar^0^\dagger p^0^\dagger>
 ****************************************************/
...
int main(int argc, char **argv) {
  ...
  double *work=NULL;
  fermion_propagator_type *fp1=NULL, *fp2=NULL, *fp3=NULL, *fpaux=NULL, *uprop=NULL, *dprop=NULL;
  spinor_propagator_type *sp1=NULL, *sp2=NULL, *sp3=NULL, *sp_aux=NULL;
...
//#endif 
  ...
  while ((c = getopt(argc, argv, "Sah?vgf:F:P:s:m:")) != -1) {
    ...
    case 'a':
      write_ascii = 1;
      fprintf(stdout, "# [sigma_ps_2_KN_sequential_v4_mpi] will write in ascii format\n");
      break;
    case 'F':
      if(strcmp(optarg, "Wilson") == 0) {
        ...
        fprintf(stderr, "[sigma_ps_2_KN_sequential_v4_mpi] Error, unrecognized fermion type\n");
        EXIT(145);
      }
      fprintf(stdout, "# [sigma_ps_2_KN_sequential_v4_mpi] will use fermion type %s ---> no. %d\n", optarg, fermion_type);
      break;
    case 'g':
      do_gt = 1;
      fprintf(stdout, "# [sigma_ps_2_KN_sequential_v4_mpi] will perform gauge transform\n");
      break;
    case 's':
      use_lattice_momenta = 1;
      fprintf(stdout, "# [sigma_ps_2_KN_sequential_v4_mpi] will use lattice momenta\n");
      break;
    case 'P':
      snk_momentum_filename_set = 1;
      strcpy(snk_momentum_filename, optarg);
      fprintf(stdout, "# [sigma_ps_2_KN_sequential_v4_mpi] will use nucleon momentum file %s\n", snk_momentum_filename);
      break;
    case 'S':
      smear_seq_source = 1;
      fprintf(stdout, "# [sigma_ps_2_KN_sequential_v4_mpi] will smear sequential soucre\n");
      break;
    case 'h':
    case '?':
    default:
  ...
  fprintf(stderr, "[sigma_ps_2_KN_sequential_v4_mpi] Error, 2-,3-dim. parallel version not yet implemented; exit\n");
  EXIT(1);
#endif
...
#else
  fprintf(stdout, "[sigma_ps_2_KN_sequential_v4_mpi] Warning, resetting global thread number to 1\n");
  g_num_threads = 1;
#endif
  ...
  if(N_Jacobi>0) {
    ...
    if(N_ape > 0) {
      if(g_cart_id==0) fprintf(stdout, "# [sigma_ps_2_KN_sequential_v4_mpi] APE smearing gauge field with paramters N_APE=%d, alpha_APE=%e\n", N_ape, alpha_ape);
#ifdef OPENMP
  ...
  g_spinor_field = NULL;
    ...
    for(imom=0;imom<rel_momentum_no; imom++) {
      ...
      for(timeslice=0; timeslice<T; timeslice++)
      {
         ...
         *
         *   uprop = S_u
         *   dprop = S_seq = S_s g5 S_d with relative momentum
         *   fp1   = C Gamma_1 S_u
...
#pragma omp parallel private (ix,iix,icomp,threadid) \
        firstprivate (fermion_type,gamma_component,connq,\
            gamma_component_sign,VOL3,g_spinor_field,fp1,fp2,fp3,fpaux,uprop,dprop,\
            sp1,sp2,sp3,sp_aux,timeslice)
...
*/
            ...
            _sp_eq_sp_ti_re(sp1[threadid], sp_aux[threadid], -1.);
            ...
            _sp_eq_sp_ti_re(sp2[threadid], sp_aux[threadid], -1.);
            ...
            _sp_eq_sp_ti_re(sp3[threadid], sp_aux[threadid], -1.);
            ...
            _sp_eq_sp_ti_re(sp2[threadid], sp1[threadid], gamma_component_sign[icomp] * _TWO_OVER_SQRT3);
...
// #endif
    ...
    free(dprop);
    free(sprop);
    free(fp1); free(fp2); free(fp3); free(fpaux);
    ...
    if( (buffer = malloc(items*sizeof(double))) == NULL ) {
      fprintf(stderr, "[sigma_ps_2_KN_sequential_v4_mpi] Error, could not allocate buffer; exit\n");
      EXIT(153);
    }
    ...
    if( status != MPI_SUCCESS ) {
      fprintf(stderr, "[sigma_ps_2_KN_sequential_v4_mpi] Error from MPI_Gather; exit\n");
      EXIT(154);
    }
#else
    ...
    if (g_cart_id == 0) {  
      ...
      ofs = fopen(filename, "w");
      if(ofs == NULL) {
        fprintf(stderr, "[sigma_ps_2_KN_sequential_v4_mpi] Error, could not open file %s for writing\n", filename);
        EXIT(3);
      }
  ...
  if(g_cart_id==0) {
    ...
    fprintf(stdout, "# [sigma_ps_2_KN_sequential_v4_mpi] %s# [sigma_ps_2_KN_sequential_v4_mpi] end fo run\n", ctime(&g_the_time));
    fflush(stdout);
    fprintf(stderr, "# [sigma_ps_2_KN_sequential_v4_mpi] %s# [sigma_ps_2_KN_sequential_v4_mpi] end fo run\n", ctime(&g_the_time));
    fflush(stderr);
  }
