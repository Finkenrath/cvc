...
int main(int argc, char **argv) {
  ...
  while ((c = getopt(argc, argv, "wWah?vgf:t:m:o:")) != -1) {
    ...
    case 't':
      ...
      fprintf(stdout, "\n# [avc_exact2_lowmem_pspace] will use %d threads in spacetime loops\n", num_threads);
      break;
    case 'a':
      write_ascii = 1;
    ...
    case 'm':
      mms = 1;
      mass_id = atoi(optarg);
      fprintf(stdout, "\n# [avc_exact2_lowmem_pspace] will read propagators in MMS format with mass id $d\n", mass_id);
      break;
    case 'o':
  ...
  sprintf(filename, "avc2_v_ct.%.4d", Nconf);
