#######################################################################
#
# make_del_contract_code_inline.sh
#
...
echo "static inline void _sp_eq_fp_del_contract${what}_fp(spinor_propagator_type _r, fermion_propagator_type _u, fermion_propagator_type _v) {"
# -------------------------------------------------------------------------------------------------

if [ "X$what" == "X13" ]; then
# contract 1 3
for((s2=0;s2<$N_SPINOR_DIM;s2++)); do
  for((b3=0;b3<$N_COLOR_DIM;b3++)); do
 
    for((s1=0;s1<$N_SPINOR_DIM;s1++)); do
      for((a3=0;a3<$N_COLOR_DIM;a3++)); do
