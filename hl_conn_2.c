...
int main(int argc, char **argv) {
  ...
  int count, hidx;
  int filename_set = 0;
  int timeslice, mms1=0, mms2=0, mms2_min=0;
  int light_nomms=0;
  int x0, x1, x2, ix, idx;
  int VOL3;
  int n_c=1, n_s=4;
...
*/
      ...
      /* write/add to file */
      if(g_cart_id==0) {
        sprintf(filename, "charged.%.4d.%.2d.%.2d.%.2d", Nconf, timeslice, k, k);
        if(count==0) {
          ofs=fopen(filename, "w");
        } else {
          ofs=fopen(filename, "a");
        ...
        fprintf(stdout, "# writing correlators to file %s\n", filename);
        fprintf(ofs, "# %5d%3d%3d%3d%3d%15.8e%15.8e%15.8e%3d%3d\n", 
          Nconf, T_global, LX, LY, LZ, g_kappa, mms_masses[k], mms_masses[k], -sigmalight, -sigmaheavy);
        for(idx=0; idx<K; idx++) {
          for(ll=0; ll<4; ll++) {
            x1 = (0+timeslice) % T_global;
            i = 2* ( (x1/T)*4*K*T + ll*K*T + idx*T + x1%T ) + isimag[idx];
        ...
        if(j==0) {
          ...
          if(Nlong>-1) {
            ...
            for(i=0; i<n_s*n_c; i++) {
              ...
              if(g_cart_id==0) fprintf(stdout, "# fuzzing prop. with Nlong=%d, N_APE=%d, alpha_APE=%f\n", Nlong, N_ape, alpha_ape);
              xchange_field_timeslice(g_spinor_field[i+hidx]);
              Fuzz_prop2(gauge_field_f, g_spinor_field[i+hidx], work, Nlong);
            }
          } else {
            /* smeared-local */
            ll = 2;
...
*/
      ...
      /* write/add to file */
      if(g_cart_id==0) {
        sprintf(filename, "charged.%.4d.%.2d.%.2d.%.2d", Nconf, timeslice, mms1, k);
        if(count==0) {
          ofs=fopen(filename, "w");
        } else {
          ofs=fopen(filename, "a");
        ...
        fprintf(stdout, "# writing correlators to file %s\n", filename);
        fprintf(ofs, "# %5d%3d%3d%3d%3d%15.8e%15.8e%15.8e%3d%3d\n", 
          Nconf, T_global, LX, LY, LZ, g_kappa, mms_masses[mms1], mms_masses[k], -sigmalight, -sigmaheavy);
        for(idx=0; idx<K; idx++) {
          for(ll=0; ll<4; ll++) {
            x1 = (0+timeslice) % T_global;
            i = 2* ( (x1/T)*4*K*T + ll*K*T + idx*T + x1%T ) + isimag[idx];
