 ...
 * TODO:
 * DONE:
 * - corrected the sample average over stochastic sources
 * - added if-clause for application of BHn
 * CHANGES:
 *********************************************************************************/
...
#  endif
    ...
    if(count%Nsave == 0) {
      ...
      sprintf(filename, "vp_disc_hpe%.2d_X.%.4d.%.4d", hpe_order, Nconf, count);
      sprintf(contype, "cvc-disc-hpe-loops-to-%2d-stoch-%2d-X", hpe_order, hpe_order+2);
      ...
      sprintf(filename, "vp_disc_hpe%.2d_P.%.4d.%.4d", hpe_order, Nconf, count);
      sprintf(contype, "cvc-disc-hpe-loops-to-%2d-stoch-%2d-P", hpe_order, hpe_order+2);
