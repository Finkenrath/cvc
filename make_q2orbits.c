...
int make_qid_lists(int *q2id, int *qhat2id, double **q2list, double **qhat2list, int *q2count, int *qhat2count) {
  ...
  if(qlist_aux == (double*)NULL) {
    fprintf(stderr, "\n[make_qid_lists] Error, could not allocate memory for qlist_aux\n");
    return(201);
  }
   ...
   ***************************************************************/
if( q2id != NULL ) {
  printf("\n# [make_q2_lists] start creating q2id ...\n");
...
} else {  // of if q2id != NULL ...
  printf("\n# [make_q2_lists] will not make q2id list; reason: q2id == NULL\n");
}

  /***************************************************************
   * make the qhat2 lists
   ***************************************************************/
if(qhat2id != NULL) {
  printf("\n# [make_q2_lists] start creating q2hatid ...\n");

  for(ix=0; ix<VOLUME/8; ix++) qlist_aux[ix] = -1.;

  count = -1;
  for(x0=0; x0 <= T/2; x0++) {
    qhat[0] = 2. * sin( M_PI / (double)T  * (double)(x0) );
  for(x1=0; x1 <= LX/2; x1++) {
  ...
  for(ix=0;ix<VOLUME;ix++) { qhat2id[ix] = -1; }
  ...
  for(x3=0; x3 <= LX/2; x3++) {
    ...
    i = -1;
    while(fabs(q2-qlist_aux[++i]) > _Q2EPS);
    // all points connected to that (x0,x1,x2,x3)
    ix   = g_ipt[  x0][   x1][   x2][   x3];
    qhat2id[ix] = i;
    ix   = g_ipt[  x0][   x1][   x2][(LX-x3)%LX];
    qhat2id[ix] = i;
    ix   = g_ipt[  x0][   x1][(LX-x2)%LX][   x3];
    qhat2id[ix] = i;
    ix   = g_ipt[  x0][   x1][(LX-x2)%LX][(LX-x3)%LX];
    qhat2id[ix] = i;
    ix   = g_ipt[  x0][(LX-x1)%LX][   x2][   x3];
    qhat2id[ix] = i;
    ix   = g_ipt[  x0][(LX-x1)%LX][   x2][(LX-x3)%LX];
    qhat2id[ix] = i;
    ix   = g_ipt[  x0][(LX-x1)%LX][(LX-x2)%LX][   x3];
    qhat2id[ix] = i;
    ix   = g_ipt[  x0][(LX-x1)%LX][(LX-x2)%LX][(LX-x3)%LX];
    qhat2id[ix] = i;
    ix   = g_ipt[(T-x0)%T][   x1][   x2][   x3];
    qhat2id[ix] = i;
    ix   = g_ipt[(T-x0)%T][   x1][   x2][(LX-x3)%LX];
    qhat2id[ix] = i;
    ix   = g_ipt[(T-x0)%T][   x1][(LX-x2)%LX][   x3];
    qhat2id[ix] = i;
    ix   = g_ipt[(T-x0)%T][   x1][(LX-x2)%LX][(LX-x3)%LX];
    qhat2id[ix] = i;
    ix   = g_ipt[(T-x0)%T][(LX-x1)%LX][   x2][   x3];
    qhat2id[ix] = i;
    ix   = g_ipt[(T-x0)%T][(LX-x1)%LX][   x2][(LX-x3)%LX];
    qhat2id[ix] = i;
    ix   = g_ipt[(T-x0)%T][(LX-x1)%LX][(LX-x2)%LX][   x3];
    qhat2id[ix] = i;
    ix   = g_ipt[(T-x0)%T][(LX-x1)%LX][(LX-x2)%LX][(LX-x3)%LX];
    qhat2id[ix] = i;
  }}}}
  ...
  *qhat2count = count+1;
  ...
  printf("\n# [make_q2_lists] will not make qhat2id list; reason: qhat2id == NULL\n");
}
