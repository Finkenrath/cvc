...
void usage() {
  fprintf(stdout, "Code to check the 5th order HPE\n");
...
int main(int argc, char **argv) {
  ...
  hpe3_coeff   = -768. * g_kappa*g_kappa*g_kappa * mutilde2 / onepmutilde2 / onepmutilde2 / onepmutilde2; 
  ...
  for(sid=0; sid<12; sid++) {
    ...
    BH2(g_spinor_field[2], g_spinor_field[1]);
    ...
    disc[0] += 2. * g_kappa * w.re;
    disc[1] += 2. * g_kappa * w.im;
  }
  ...
  sprintf(filename, "tr_BH2B_ix0.%.4d", Nconf);
