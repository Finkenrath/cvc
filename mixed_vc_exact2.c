...
int main(int argc, char **argv) {
  ...
  for(ia=0; ia<n_c; ia++) {
    ...
    for(i=0; i<4; i++) {
      get_filename(filename, 0, n_c*i+ia, 1);
      ...
      for(ix=0; ix<VOLUME; ix++) {
        for(mu=0; mu<4; mu++) {
          ...
          for(ir=0;ir<4;ir++) {
            ...
            _co_eq_fv_dag_ti_fv(&w, &g_spinor_field[4+vpsource[nu][ir]][_GSI(ix)], spinor2);
            if(!visimag[nu]) {
            ...
            //fprintf(stdout, "%3d%8d%3d%3d\t%e\t%e\n", nu, ix, ir, ia, 
            //    vssource[nu][ir] * w.re, vssource[nu][ir] * w.im);
          }
        }  // of mu
      }  // of ix
    }  // of nu
      ...
      for(ix=0; ix<VOLUME; ix++) {
        for(mu=0; mu<4; mu++) {
          ...
          for(ir=0;ir<4;ir++) {
            ...
            _co_eq_fv_dag_ti_fv(&w, &g_spinor_field[4+vpsource[nu][ir]][_GSI(ix)], spinor1);
            if(!visimag[nu]) {
            ...
            _fv_eq_gamma_ti_fv(spinor1, 5, spinor2);
            _co_eq_fv_dag_ti_fv(&w, &g_spinor_field[4+vpsource[nu][ir]][_GSI(g_iup[ix][mu])], spinor1);
            if(!visimag[nu]) {
              conn[_GWI(4*mu+nu,ix,VOLUME)  ] -= vssource[nu][ir] * w.re;
              conn[_GWI(4*mu+nu,ix,VOLUME)+1] -= vssource[nu][ir] * w.im;
            } else {
      ...
      for(ix=0; ix<VOLUME; ix++) {
        for(mu=0; mu<4; mu++) {
          ...
          for(ir=0;ir<4;ir++) {
            _fv_eq_cm_ti_fv(spinor1, U_, &g_spinor_field[ir][_GSI(g_iup[ix][mu])]);
            _fv_eq_gamma_ti_fv(spinor2, mu, spinor1);
            _co_eq_fv_dag_ti_fv(&w, &g_spinor_field[4+apsource[nu][ir]][_GSI(ix)], spinor2);
            if(!aisimag[nu]) {
            ...
            _fv_eq_cm_dag_ti_fv(spinor1, U_, &g_spinor_field[ir][_GSI(ix)]);
            _fv_eq_gamma_ti_fv(spinor2, mu, spinor1);
            _co_eq_fv_dag_ti_fv(&w, &g_spinor_field[4+apsource[nu][ir]][_GSI(g_iup[ix][mu])], spinor2);
            if(!aisimag[nu]) {
              conn3[_GWI(4*mu+nu,ix,VOLUME)  ] -= assource[nu][ir] * w.re;
              conn3[_GWI(4*mu+nu,ix,VOLUME)+1] -= assource[nu][ir] * w.im;
            } else {
