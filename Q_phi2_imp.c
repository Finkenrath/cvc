 ...
 * TODO:
 * - improve Hopping_iter
 *   (1) at (sigma_n, mu_n) check, if (sigma_(n+1), mu_(n+1)) = (-sigma_n, mu_n),
 *       if so, skip multiplication
 *   (2) avoid calculation of forward loops, take complex conjugate of backward loops
 *   (3) reduce loops before starting Hopping_iter
 * DONE:
...
/*  fprintf(stdout, "before: xnew = %5d\n", xnew); */
    for(l1=0; l1<deg; l1++) {
      ...
      if(l1<deg-1) {
        if( abs(loop_tab[l][l1]-loop_tab[l][l1+1]) == 4 ) {
/*          fprintf(stdout, "skipping l1-pair %d, %d for loop %d\n", l1, l1+1, l); */
          if(l1==deg-2) {
        ...
        if( loop_tab[l][l1]>3 && perm[loop_tab[l][l1]%4]==mu ) {
/*          fprintf(stdout, "skipping last l1=%d for loop %d\n", l1, l); */
          u3=u1; u1=u2; u2=u3;
          continue;
        }
      }
