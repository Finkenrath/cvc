#######################################################################
#
# make_eps_contract_code_inline.sh
#
...
what=$1
echo "#ifndef _EPS_CONTRACT_${what}_INLINE_H"
echo "#define _EPS_CONTRACT_${what}_INLINE_H"

echo "// spinor dimension: $N_SPINOR_DIM"
echo "// color dimension: $N_COLOR_DIM"
echo ""

echo "static inline void _fp_eq_fp_eps_contract${what}_fp(fermion_propagator_type _r, fermion_propagator_type _u, fermion_propagator_type _v) {"

if [ "X$what" == "X13" ]; then
# contract 1 3
for((s2=0;s2<$N_SPINOR_DIM;s2++)); do
  for((jperm=0;jperm<$NUM_PERM;jperm++)); do
    ...
    for((s1=0;s1<$N_SPINOR_DIM;s1++)); do
      for((iperm=0;iperm<$NUM_PERM;iperm++)); do
