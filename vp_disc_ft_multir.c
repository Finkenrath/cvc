 ...
 *
 * PURPOSE:
 * - based of vp_disc_ft.c
 * - _ATTENTION_: the model function data is already normalized
 *   in pidisc_modelN
 * - difference to vp_disc_ft.c: use several radii (max. 4) at once
 * TODO:
 * DONE:
 * CHANGES:
 *********************************************************************************/
...
int main(int argc, char **argv) {
  ...
  if(NRmax > 4) {
    if(g_proc_id==0) fprintf(stdout, "NRmax must be le 4\n");
    usage();
  }
...
#  endif
    ...
    for(nu=0; nu<4; nu++) {
      ...
      for(x3=-LZ+1; x3<LZ; x3++) {
        ...
        rsqr = sqrt( (double)(x1*x1) + (double)(x2*x2) + (double)(x3*x3) );
        ft_flag[0] = (rsqr-ft_rmax[0] <= _Q2EPS);
        ft_flag[1] = (rsqr-ft_rmax[1] <= _Q2EPS);
        ft_flag[2] = (rsqr-ft_rmax[2] <= _Q2EPS);
        ft_flag[3] = (rsqr-ft_rmax[3] <= _Q2EPS);
        if( !( ft_flag[0] || ft_flag[1] || ft_flag[2] || ft_flag[3] ) ) continue;
