...
int main(int argc, char **argv) {
  ...
  init_to_zero<<<(4*FFTW_LOC_VOLUME-1+threadsPerBlock)/threadsPerBlock, threadsPerBlock>>>(d_data, 4*FFTW_LOC_VOLUME);
  ...
  for(gid=g_gaugeid; gid<=g_gaugeid2; gid++) {
    ...
    for(sid2=sid1+1; sid2<nprop; sid2++) {
      ...
      for(count=0; count<8*FFTW_LOC_VOLUME; count++) h_ddata[count] += (double)h_sdata[count];
    }  /* of loop on sid2 */
    }  /* of loop on sid1 */
     ...
     ************************************************/
    ratime = (double)clock() / CLOCKS_PER_SEC;
    sprintf(filename, "pi_ud_r3.%4d", gid);
    ofs = fopen(filename, "w");
    if (ofs==NULL) {
      ...
      for(it=t_start; it<Tsub; it++) {
        ...
        for(iz=z_start; iz<Lsub; iz++) {
          ...
          fprintf(ofs, "%3d%3d%3d%3d%3d%16.7e%25.16e%25.16e\n", mu, it, ix, iy, iz, r2,
            h_ddata[_GWI(mu,ip,FFTW_LOC_VOLUME)], h_ddata[_GWI(mu,ip,FFTW_LOC_VOLUME)+1]);
          ip++;
        }}}
      }
    }
