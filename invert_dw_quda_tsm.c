/****************************************************
 *
 * invert_dw_quda_tsm.c
 *
 * Mon May  7 12:25:03 EEST 2012
 *
 * PURPOSE:
...
int main(int argc, char **argv) {
  ...
  int full_orbit = 0;
  int smear_source = 0;
  char filename[200], source_filename[200], line[200];
...
#endif
  momentum_info_type momentum_info;
...
//#endif
  ...
  while ((c = getopt(argc, argv, "soch?vgf:n:b:S:R:p:")) != -1) {
    switch (c) {
    case 'v':
      g_verbose = 1;
      break;
    ...
    case 'R':
      ...
      break;
    case 'p':
      momentum_info.filename_set = 1;
      strcpy(momentum_info.filename, optarg);
      fprintf(stdout, "# [] using momentum filename %s\n", momentum_info.filename);
      break;
    case 'h':
    case '?':
    default:
...
#else
  fprintf(stdout, "[invert_dw_quda] Warning, resetting global number of threads to 1\n");
  g_num_threads = 1;
#endif
  ...
  /***************************************************************************
   * read the relative momenta q to be used, or use all
   ***************************************************************************/
  if(momentum_info.filename_set) {
    ofs = fopen(momentum_info.filename, "r");
    if(ofs == NULL) {
      fprintf(stderr, "[] Error, could not open file %s for reading\n", momentum_info.filename);
      return(6);
    }
    momentum_info.number = 0;
    while( fgets(line, 199, ofs) != NULL) {
      if(line[0] != '#') {
        momentum_info.number++;
      }
    }
    if(momentum_info.number == 0) {
    ...
    rewind(ofs);
    momentum_info.list    = (int**)malloc(  momentum_info.number * sizeof(int*));
    momentum_info.list[0] = (int* )malloc(3*momentum_info.number * sizeof(int));
    for(i=1;i<momentum_info.number;i++) { momentum_info.list[i] = momentum_info.list[i-1] + 3; }
    count=0;
    while( fgets(line, 199, ofs) != NULL) {
      if(line[0] != '#') {
        sscanf(line, "%d%d%d", momentum_info.list[count], momentum_info.list[count]+1, momentum_info.list[count]+2);
        count++;
      }
    }
    fclose(ofs);
    if(g_cart_id==0) fprintf(stdout, "# [] current momentum list:\n");
    for(i=0;i<momentum_info.number;i++) {
      if(momentum_info.list[i][0] < 0 ) momentum_info.list[i][0] += LX_global;
      if(momentum_info.list[i][1] < 0 ) momentum_info.list[i][1] += LY_global;
      if(momentum_info.list[i][2] < 0 ) momentum_info.list[i][2] += LZ_global;
      if(g_cart_id==0) fprintf(stdout, "\t%3d |%3d%3d%3d\n", i, momentum_info.list[i][0], momentum_info.list[i][1], momentum_info.list[i][2]);
    }
  } else {
    momentum_info.number = VOL3;
  }
...
//      }
       ...
       * contractions
       ***********************************************/
      // rebuild the 4-dim. source
#ifndef HAVE_QUDA
      if(rotate_gamma_basis) {
        if(g_cart_id==0) fprintf(stdout, "# [invert_dw_quda] Attention: gamma_2 - rotation of source back to original verson in non-QUDA case\n");
        for(ix=0;ix<L5*VOLUME;ix++) { 
          _fv_eq_gamma_ti_fv(spinor1, 2, g_spinor_field[0]+_GSI(ix));
          _fv_eq_fv(g_spinor_field[0]+_GSI(ix), spinor1);
        }
...
#endif
      spinor_4d_to_5d_inv(g_spinor_field[0], g_spinor_field[0]);
      //sprintf(filename, "source.ascii.%.2d.%.2d", g_nproc, g_cart_id);
      //ofs = fopen(filename, "w");
      //printf_spinor_field(g_spinor_field[0], ofs);
      //fclose(ofs);

      check_error(contract_disc_tqdep_v2(g_spinor_field[0], g_spinor_field[1], &momentum_info, isc), "contract_disc_tqdep_v2", NULL, 23);
