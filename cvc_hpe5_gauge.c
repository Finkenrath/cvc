 ...
 *
 * PURPOSE:
 * - calculate the contracted current J_\mu(x) in position space
...
#  endif
    ...
    for(mu=0; mu<4; mu++) {
      for(ix=0; ix<VOLUME; ix++) {
        disc[_GWI(mu,ix,VOLUME)  ] += disc2[_GWI(mu,ix,VOLUME)  ];
        disc[_GWI(mu,ix,VOLUME)+1] += disc2[_GWI(mu,ix,VOLUME)+1];
      }
    }
...
*/
	...
	_co_eq_co_ti_co(&w1, (complex*)(work+_GWI(mu,ix,VOLUME)), (complex*)(work+_GWI(4+nu,ix,VOLUME)));
	_co_pl_eq_co_ti_co((complex*)(work+_GWI(8+4*mu+nu,ix,VOLUME)), &w1, &w);
/*	cp1++; cp2++; cp3++; */
      }
      }
      }
    ...
    retime = (double)clock() / CLOCKS_PER_SEC;
#endif
    if(g_cart_id==0) fprintf(stdout, "time to for Fourier transformation: %e seconds\n", retime-ratime);
  ...
  fnorm = 1. / (double)(T_global*LX*LY*LZ) / (double)count;
  fprintf(stdout, "fnorm = %e\n", fnorm);
  for(mu=0; mu<16; mu++) {
    for(ix=0; ix<VOLUME; ix++) {
      work[_GWI(8+mu,ix,VOLUME)  ] *= fnorm;
      work[_GWI(8+mu,ix,VOLUME)+1] *= fnorm;
    }
  }

  sprintf(filename, "cvc_hpe5_gauge.%.4d.%.2d", Nconf, count);
