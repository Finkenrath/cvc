 ...
 * invert_dw_quda_v3.c
 *
 * Fri May 18 15:43:46 EEST 2012
 *
 * PURPOSE:
 * - like invert_dw_quda, but should smear in CPUs in parallel to inversions on GPUs
 * - would use (N_threas-for-smearing + 1 threads for GPUs) threads
 * TODO:
 *
 * DONE:
 * CHANGES:
...
void usage() {
  fprintf(stdout, "Code to invert D_dw\n");
...
int main(int argc, char **argv) {
  ...
  double cpu_ratime, cpu_retime, gpu_ratime, gpu_retime;
...
#pragma omp parallel for private(ix,iy)
  ...
  for(i=0; i<no_fields; i++) alloc_spinor_field(&g_spinor_field[i], (VOLUME+RAND)*L5);
...
//  for(isc=g_source_index[0]; isc<=g_source_index[0]; isc++)
  {
    ...
    for(imom=0; imom<source_momentum_runs; imom++) {
      ...
      if(g_read_source == 0) {  // create source
        switch(g_source_type) {
          case 0:
            ...
            for(ix=0;ix<L5*VOLUME;ix++) { _fv_eq_zero(smearing_spinor_field[0]+ix); }
            if(have_source_flag) {
              if(g_source_momentum_set) {
                ...
                smearing_spinor_field[0][_GSI(g_ipt[lsl0][lsl1][lsl2][lsl3]) + 2*(n_c*ispin+icol)  ] = cos(phase);
                smearing_spinor_field[0][_GSI(g_ipt[lsl0][lsl1][lsl2][lsl3]) + 2*(n_c*ispin+icol)+1] = sin(phase);
              } else {
                smearing_spinor_field[0][_GSI(g_ipt[lsl0][lsl1][lsl2][lsl3]) + 2*(n_c*ispin+icol)  ] = 1.;
              }
            }
            if(g_source_momentum_set) {
          ...
          case 2:
            ...
            if(g_coherent_source==1) {
              ...
              status = prepare_coherent_timeslice_source(smearing_spinor_field[0], gauge_field_smeared, g_coherent_source_base, g_coherent_source_delta, VOLUME, g_rng_state, 1);
              if(status != 0) {
                fprintf(stderr, "[invert_dw_quda] Error from prepare source, status was %d\n", status);
                EXIT(123);
              }
              check_error(prepare_coherent_timeslice_source(smearing_spinor_field[0], gauge_field_smeared, g_coherent_source_base, g_coherent_source_delta, VOLUME, g_rng_state, 1),
              ...
              if(g_coherent_source==2) {
                ...
                check_error(prepare_timeslice_source(smearing_spinor_field[0], gauge_field_smeared, timeslice, VOLUME, g_rng_state, 1),
                    "prepare_timeslice_source", NULL, 123);
              } else {
                if(g_cart_id==0) fprintf(stdout, "# [invert_dw_quda] Creating timeslice source\n");
                check_error(prepare_timeslice_source(smearing_spinor_field[0], gauge_field_smeared, g_source_timeslice, VOLUME, g_rng_state, 1),
          ...
          case 3:
            ...
            check_error( prepare_timeslice_source_one_end(smearing_spinor_field[0], gauge_field_smeared, source_timeslice, source_momentum, isc%n_s, g_rng_state, \
          ...
          case 4:
            ...
            fprintf(stdout, "# [invert_dw_quda] Creating timeslice source for one-end-trick\n");
            check_error(prepare_timeslice_source_one_end_color(smearing_spinor_field[0], gauge_field_smeared, source_timeslice, source_momentum,\
          ...
          case 5:
            ...
            check_error( prepare_sequential_point_source (smearing_spinor_field[0], isc, sl0, g_seq_source_momentum, 
        ...
        switch(g_source_type) {
          case 0:  // point source
            ...
            check_error(read_lime_spinor(smearing_spinor_field[0], source_filename, 0), "read_lime_spinor", NULL, 115);
            break;
      ...
      if(g_write_source) {
        check_error(write_propagator(smearing_spinor_field[0], source_filename, 0, g_propagator_precision), "write_propagator", NULL, 27);
      }

      cpu_retime = CLOCK;
      if(g_cart_id == 0) fprintf(stdout, "# [invert_dw_quda] time for source initalization = %e seconds\n", cpu_retime-cpu_ratime);
 ...
 ***********************************************************************************************/
      if(dummy_flag==0) {
        strcpy(source_filename_write, source_filename);
      }

      if(dummy_flag > 0) {
        // copy only if smearing has been done; otherwise do not copy
        memcpy((void*)(g_spinor_field[2]), (void*)(smearing_spinor_field[1]), 24*VOLUME*L5*sizeof(double));
      }
      abort = 0;

#pragma omp parallel num_threads(2) default(none) \
  private(threadid, threadid1,_2_kappa, ix, iy, iix, is, status, norm, norm2, \
      filename, cpu_ratime, cpu_retime, gpu_ratime, gpu_retime) \
      \
  shared(g_read_source, smear_source, N_Jacobi, kappa_Jacobi, convert_sign,\
      isc, rotate_gamma_basis, g_cart_id, g_num_threads, dummy_flag, abort, check_residuum, g_propagator_precision, \
      source_filename_write, source_filename) \
      \
  firstprivate(inv_param, gauge_param, ofs, gauge_field_smeared, smearing_spinor_field,g_spinor_field, \
      stdout, stderr, VOLUME, L5, RAND, gamma_permutation, gamma_sign, g_kappa5d)
{
  threadid = omp_get_thread_num();

  fprintf(stdout, "# [] proc%.2d thread%.2d\n", g_cart_id, threadid);

  if(threadid == 0) {  // cpu thread
      ...
      if( ( !g_read_source || (g_read_source && smear_source ) ) && N_Jacobi > 0 ) {
        ...
        if( Jacobi_Smearing_Step_one_threads(gauge_field_smeared, smearing_spinor_field[0], smearing_spinor_field[1], \
              N_Jacobi, kappa_Jacobi) != 0 ) {
          abort = 1;
       ...
       ***********************************************/
      if(convert_sign == 0) {
        spinor_4d_to_5d_threaded(smearing_spinor_field[0], smearing_spinor_field[0]);
      }  else if(convert_sign == 1 || convert_sign == -1) {
        spinor_4d_to_5d_sign_threaded(smearing_spinor_field[0], smearing_spinor_field[0], convert_sign);
      }


      // reorder, multiply with g2
      memset(smearing_spinor_field[1], 0, 24*(VOLUME+RAND)*L5*sizeof(double));

      if(rotate_gamma_basis) {
#pragma omp parallel for default(none) num_threads(g_num_threads) private(ix,iy) firstprivate(smearing_spinor_field, VOLUME, L5, gamma_permutation, gamma_sign)
        for(ix=0; ix<VOLUME; ix++) {
...
#pragma omp parallel for default(none) num_threads(g_num_threads) private(ix,iy) firstprivate(smearing_spinor_field, VOLUME, L5)
        for(ix=0; ix<VOLUME; ix++) {
          iy = lexic2eot_5d(0, ix);
...
#pragma omp parallel for default(none) num_threads(g_num_threads) private(ix,iy) firstprivate(smearing_spinor_field, VOLUME, L5)
      ...
      cpu_retime = CLOCK;
      if(g_cart_id==0) fprintf(stdout, "# [invert_dw_quda] wall time source preparation = %e seconds\n", cpu_retime - cpu_ratime);

      //sprintf(filename, "%s.final.ascii.%.2d", source_filename, g_cart_id);
      //ofs = fopen(filename, "w");
      //printf_spinor_field_5d(smearing_spinor_field[0], ofs);
      //fclose(ofs);
   
  } else if(threadid == 1 && dummy_flag>0) {  // else branch on threadid; gpu thread

      fprintf(stdout, "# [] proc%.2d thread%.2d starting inversion\n", g_cart_id, threadid);

      //sprintf(filename, "%s.ascii.%.2d", source_filename_write, g_cart_id);
      //ofs = fopen(filename, "w");
      ...
      if(g_cart_id==0) {
        ...
        fprintf(stdout, "# [invert_dw_quda] wall time source inversion   = %e seconds\n", gpu_retime - gpu_ratime);
      ...
      if(abort) {
        EXIT_WITH_MSG(96, "[invert_dw_quda] Error in parallel region\n");
      }
      ...
      if(dummy_flag>0) {
        if(inv_param.mass_normalization == QUDA_KAPPA_NORMALIZATION) {
          ...
          #pragma omp parallel for default(none) private(ix) firstprivate(_2_kappa,g_spinor_field, VOLUME, L5) num_threads(g_num_threads)
        ...
        // reorder, multiply with g2
        for(is=0;is<L5;is++) {
          #pragma omp parallel for default(none) private(ix,iix,iy) firstprivate(g_spinor_field, VOLUME, L5) num_threads(g_num_threads) shared(is)
          for(ix=0; ix<VOLUME; ix++) {
        ...
        if(rotate_gamma_basis) {
          #pragma omp parallel for default(none) private(ix) firstprivate(g_spinor_field,VOLUME, L5, gamma_permutation, gamma_sign) num_threads(g_num_threads)
          for(ix=0; ix<VOLUME*L5; ix++) {
            _fv_eq_gamma_ti_fv(g_spinor_field[1]+_GSI(ix), 2, g_spinor_field[2]+_GSI(ix));
          }
        } else {
          #pragma omp parallel for default(none) private(ix) firstprivate(g_spinor_field,VOLUME,L5) num_threads(g_num_threads)
          for(ix=0; ix<VOLUME*L5;ix++) {
        ...
        cpu_retime = CLOCK;
        if(g_cart_id==0) fprintf(stdout, "# [invert_dw_quda] wall time source post processing = %e seconds\n", cpu_retime - cpu_ratime);
        ...
        if(check_residuum) {
          cpu_ratime = CLOCK;
          memset(g_spinor_field[2], 0, 24*(VOLUME+RAND)*L5*sizeof(double));
          ...
          #pragma omp parallel for default(none) num_threads(g_num_threads) private(ix) firstprivate(g_spinor_field, VOLUME, L5)
          for(ix=0;ix<VOLUME*L5;ix++) {
          ...
          cpu_retime = CLOCK;
          if(g_cart_id == 0) fprintf(stdout, "# [invert_dw_quda] wall time residuum check = %e seconds\n", cpu_retime - cpu_ratime);
        }
         ...
         ***********************************************/
        if(convert_sign == 0) {
          spinor_5d_to_4d_threaded(g_spinor_field[1], g_spinor_field[1]);
        } else if(convert_sign == -1 || convert_sign == +1) {
          spinor_5d_to_4d_sign_threaded(g_spinor_field[1], g_spinor_field[1], convert_sign);
        }
        ...
        cpu_retime = CLOCK;
        if(g_cart_id == 0) fprintf(stdout, "# [invert_dw_quda] wall time writing propagator = %e seconds\n", cpu_retime - cpu_ratime);
      ...
      // copy new original source (in cvc format) to g_spinor_field[0]
      memcpy(g_spinor_field[0], smearing_spinor_field[0], 24*VOLUME*L5*sizeof(double));
   ...
   * perform the inversion
   ***********************************************/
  if(g_cart_id==0) fprintf(stdout, "# [invert_dw_quda] final inversion\n");
  memcpy(g_spinor_field[2], smearing_spinor_field[1], 24*VOLUME*L5*sizeof(double));
  ...
  if(g_cart_id==0) fprintf(stdout, "# [invert_dw_quda] wall time source inversion and postprocessing = %e seconds\n", \
      cpu_retime - cpu_ratime);
