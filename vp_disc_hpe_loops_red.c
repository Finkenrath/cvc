 ...
 * DONE:
 * - tested together with vp_disc_hpe_stoch against avc_disc_stochastic and
 *   avc_disc_hpe5 for orders 3, 5, 7
 * CHANGES:
 *********************************************************************************/
    ...
    for(mu=0; mu<4; mu++) {
...
#endif
        init_trace_coeff_red(&(tcf[deg]), &(tcb[deg]), &(loop_tab[deg]), 
      ...
      for(deg=hpe_order_min; deg<=hpe_order_max; deg+=2) {
        ...
        for(ix=0; ix<VOLUME; ix++) { 
          Hopping_iter_red( disc+_GWI(mu,ix,VOLUME), disc+_GWI(mu,ix,VOLUME), tcf[deg], tcb[deg], ix, mu, deg, nloop[deg], loop_tab[deg], sigma_tab[deg], shift_start[deg]);
        }
#ifdef MPI
        retime = MPI_Wtime();
#else
        retime = (double)clock() / CLOCKS_PER_SEC;
#endif
        if(g_cart_id==0) fprintf(stdout, "# time to calculate contribution from deg=%d for gaugeid %d: %e seconds\n", deg, gid, retime-ratime);
      }

    }

/*
    sprintf(filename, "vp_disc_hpe%.2d_loops_Xascii.%.4d", hpe_order_max, gid);
    ...
    fclose(ofs);
*/
    sprintf(filename, "vp_disc_hpe%.2d_loops_X.%.4d", hpe_order_max, gid);
    sprintf(contype, "cvc-disc-hpe-%.2d-to-%.2d-loops-X", hpe_order_min, hpe_order_max);
