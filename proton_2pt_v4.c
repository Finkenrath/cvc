...
//  g_source_time_slice = sx0;
  ...
  for(i=0;i<g_num_threads;i++) { create_fp(sp1+i); }
  sp2 = (spinor_propagator_type*)malloc(g_num_threads*sizeof(spinor_propagator_type));
  for(i=0;i<g_num_threads;i++) { create_fp(sp2+i); }
  ...
  firstprivate(uprop,dprop,fp1,fp2,fp3,sp1,sp2,g_spinor_field,VOLUME,fermion_type,connq)
{
  ...
  // for(ix=0;ix<2;ix++) 
  {
    ...
    // S_u x Cg5
    _fp_eq_fp_ti_Cg5(fp1[threadid], uprop[threadid], fp3[threadid]);

    // Cg5 x S_d
    _fp_eq_Cg5_ti_fp(fp2[threadid], dprop[threadid], fp3[threadid]);
    ...
    _fp_eq_fp_eps_contract13_fp(fp3[threadid], fp1[threadid], fp2[threadid]);

    // reduce to spin propagator
    _sp_eq_zero( sp1[threadid] );
    _sp_eq_fp_del_contract34_fp(sp1[threadid], uprop[threadid], fp3[threadid]);
    ...
    // reduce
    _fp_eq_zero(fp3[threadid]);
    _fp_eq_fp_eps_contract24_fp(fp3[threadid], fp1[threadid], fp2[threadid]);

    // reduce to spin propagator
    _sp_eq_zero( sp2[threadid] );
    _sp_eq_fp_del_contract23_fp(sp2[threadid], fp3[threadid], uprop[threadid]);
