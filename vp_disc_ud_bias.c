 ...
 * vp_disc_hpe_bias.c
 *
 * Wed Jan 27 16:18:00 CET 2010 
 *
 * PURPOSE:
 * - calculate the bias of the estimator for stochastic propagator part of the 
 *   disconnected contractions of the vacuum polarization; combine up- and down-part
 * TODO:
 * DONE:
 * CHANGES:
 *********************************************************************************/
  ...
  for(gid=g_gaugeid; gid<=g_gaugeid2; gid++) {
    ...
    for(sid=g_sourceid; sid<=g_sourceid2; sid+=g_sourceid_step) {
      ...
      if(count==Nsave) {
        ...
        sprintf(filename, "vp_disc_ud_bias_P.%.4d.%.4d", gid, count);
        sprintf(contype, "cvc-disc-u_and_d-stoch-bias-P");
