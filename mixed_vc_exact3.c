 ...
 * mixed_vc_exact3.c
 *
 * Thu Mar 24 10:33:57 CET 2011
 *
 * PURPOSE:
 * DONE:
 * TODO:
 * CHANGES:
 * - analog to mixed_vc_exact2, but use ll 24 fermion fields
 *   at once
 ****************************************************/
...
int main(int argc, char **argv) {
  ...
  int n_c=1;
  int source_type = 0;
  int visimag[4], aisimag[4];
  int vpsource[4][4], apsource[4][4];
  double *conn = (double*)NULL;
  double *conn2 = (double*)NULL;
  //double *conn3 = (double*)NULL;
  double vssource[4][4], assource[4][4];
  double A[4][12][24];
  double V[4][12][24];
  double B[4][12][24];
  double phase[4], q[4];
  ...
  while ((c = getopt(argc, argv, "h?vgf:p:s:")) != -1) {
    switch (c) {
    case 'v':
      verbose = 1;
      break;
    ...
    case 'p':
      ...
      break;
    case 's':
      source_type = atoi(optarg);
      fprintf(stdout, "\n# [] will use source type = %d\n", source_type);
      break;
    case 'h':
    case '?':
    default:
  ...
  if(g_cart_id==0) {
    fprintf(stdout, "\n# [mixed_vc_exact2] Using global time stamp %s", ctime(&g_the_time));
  }
  ...
  if( (source_type==1 && n_c == 3) || (source_type==0 && n_c == 1) ) {
    if(g_proc_id==0) fprintf(stdout, "WARNING: strange combination of source_type and number of colors (%d, %d)\n", source_type, n_c);
  }
   ...
   *****************************************/
  no_fields = 2*4*n_c;  // 2(up, dn) x 12 (source spin-color index)
  ...
  /********************************
   * gamma permutations at source
   ********************************/
  for(nu=0;nu<4;nu++) {
    vpsource[nu][0] = gamma_permutation[6+nu][ 0] / 6;
    vpsource[nu][1] = gamma_permutation[6+nu][ 6] / 6;
    vpsource[nu][2] = gamma_permutation[6+nu][12] / 6;
    vpsource[nu][3] = gamma_permutation[6+nu][18] / 6;
    visimag[nu]     = gamma_permutation[6+nu][ 0] % 2;

    vssource[nu][0] =  gamma_sign[6+nu][ 0];
    vssource[nu][1] =  gamma_sign[6+nu][ 6];
    vssource[nu][2] =  gamma_sign[6+nu][12];
    vssource[nu][3] =  gamma_sign[6+nu][18];


    fprintf(stdout, "gamma[%d]: (%d, %f), (%d, %f), (%d, %f), (%d, %f)\n", \
        6+nu, vpsource[nu][0], vssource[nu][0], vpsource[nu][1], vssource[nu][1],\
        vpsource[nu][2], vssource[nu][2],vpsource[nu][3], vssource[nu][3]);
    fprintf(stdout, "gamma[%d] is imag: %d\n", 6+nu, visimag[nu]);
  }

  for(nu=0;nu<4;nu++) {
    apsource[nu][0] = gamma_permutation[nu][ 0] / 6;
    apsource[nu][1] = gamma_permutation[nu][ 6] / 6;
    apsource[nu][2] = gamma_permutation[nu][12] / 6;
    apsource[nu][3] = gamma_permutation[nu][18] / 6;
    aisimag[nu]     = gamma_permutation[nu][ 0] % 2;

    assource[nu][0] =  gamma_sign[nu][ 0];
    assource[nu][1] =  gamma_sign[nu][ 6];
    assource[nu][2] =  gamma_sign[nu][12];
    assource[nu][3] =  gamma_sign[nu][18];
    fprintf(stdout, "gamma[%d]: (%d, %f), (%d, %f), (%d, %f), (%d, %f)\n", \
        nu, apsource[nu][0], assource[nu][0], apsource[nu][1], assource[nu][1],\
        apsource[nu][2], assource[nu][2],apsource[nu][3], assource[nu][3]);
    fprintf(stdout, "gamma[%d] is imag: %d\n", nu, aisimag[nu]);
  }
   ...
   ********************************/
    ...
    /* read 4*n_c up-type propagators */
    for(i=0; i<4*n_c; i++) {
      get_filename(filename, 0, i, 1);
      read_lime_spinor(g_spinor_field[i], filename, 0);
      xchange_field(g_spinor_field[i]);
    }

    /* read 4*n_c dn-type propagators */
    for(i=0; i<4*n_c; i++) {
      get_filename(filename, 0, i, -1);
      read_lime_spinor(g_spinor_field[4*n_c+i], filename, 0);
      xchange_field(g_spinor_field[4*n_c+i]);
    }
    ...
    for(nu=0; nu<4; nu++) {
      ...
      for(ix=0; ix<VOLUME; ix++) {
        ...
        for(mu=0; mu<4; mu++) {
          ...
          for(ir=0;ir<4;ir++) {
          for(ia=0; ia<n_c; ia++) {
            _fv_eq_gamma_ti_fv(spinor1, mu, &g_spinor_field[ir*n_c+ia][_GSI(ix)]);
            _fv_eq_gamma_ti_fv(spinor2, 5, spinor1);
            _co_eq_fv_dag_ti_fv(&w, &g_spinor_field[4*n_c+vpsource[nu][ir]*n_c+ia][_GSI(ix)], spinor2);
            if(!visimag[nu]) {
              conn2[_GWI(4*mu+nu,ix,VOLUME)  ]  = vssource[nu][ir] * w.re;
              conn2[_GWI(4*mu+nu,ix,VOLUME)+1]  = vssource[nu][ir] * w.im;
            } else {
              conn2[_GWI(4*mu+nu,ix,VOLUME)  ]  = vssource[nu][ir] * w.im;
              conn2[_GWI(4*mu+nu,ix,VOLUME)+1] -= vssource[nu][ir] * w.re;
            }
          }}
          ...
          for(ir=0;ir<4;ir++) {
          for(ia=0; ia<n_c; ia++) {
            _fv_eq_cm_ti_fv(spinor1, U_, &g_spinor_field[ir*n_c+ia][_GSI(g_iup[ix][mu])]);
            _fv_eq_gamma_ti_fv(spinor2, mu, spinor1);
            _fv_mi_eq_fv(spinor2, spinor1);
            _fv_eq_gamma_ti_fv(spinor1, 5, spinor2);
            _co_eq_fv_dag_ti_fv(&w, &g_spinor_field[4*n_c+vpsource[nu][ir]*n_c+ia][_GSI(ix)], spinor1);
            if(!visimag[nu]) {
              conn[_GWI(4*mu+nu,ix,VOLUME)  ] -= vssource[nu][ir] * w.re;
              conn[_GWI(4*mu+nu,ix,VOLUME)+1] -= vssource[nu][ir] * w.im;
            } else {
              conn[_GWI(4*mu+nu,ix,VOLUME)  ] -= vssource[nu][ir] * w.im;
              conn[_GWI(4*mu+nu,ix,VOLUME)+1] -= vssource[nu][ir] * w.re;
            }

            _fv_eq_cm_dag_ti_fv(spinor1, U_, &g_spinor_field[ir*n_c+ia][_GSI(ix)]);
            _fv_eq_gamma_ti_fv(spinor2, mu, spinor1);
            _fv_pl_eq_fv(spinor2, spinor1);
            _fv_eq_gamma_ti_fv(spinor1, 5, spinor2);
            _co_eq_fv_dag_ti_fv(&w, &g_spinor_field[4*n_c+vpsource[nu][ir]*n_c+ia][_GSI(g_iup[ix][mu])], spinor1);
            if(!visimag[nu]) {
              conn[_GWI(4*mu+nu,ix,VOLUME)  ] -= vssource[nu][ir] * w.re;
              conn[_GWI(4*mu+nu,ix,VOLUME)+1] -= vssource[nu][ir] * w.im;
            } else {
...
/*          
          ...
          for(ia=0; ia<n_c; ia++) {
            _fv_eq_cm_ti_fv(spinor1, U_, &g_spinor_field[ir*n_c+ia][_GSI(g_iup[ix][mu])]);
            _fv_eq_gamma_ti_fv(spinor2, mu, spinor1);
            _co_eq_fv_dag_ti_fv(&w, &g_spinor_field[4*n_c+apsource[nu][ir]*n_c+ia][_GSI(ix)], spinor2);
            if(!aisimag[nu]) {
              conn3[_GWI(4*mu+nu,ix,VOLUME)  ] -= assource[nu][ir] * w.re;
              conn3[_GWI(4*mu+nu,ix,VOLUME)+1] -= assource[nu][ir] * w.im;
            } else {
              conn3[_GWI(4*mu+nu,ix,VOLUME)  ] -= assource[nu][ir] * w.im;
              conn3[_GWI(4*mu+nu,ix,VOLUME)+1] -= assource[nu][ir] * w.re;
            }

            _fv_eq_cm_dag_ti_fv(spinor1, U_, &g_spinor_field[ir][_GSI(ix)]);
            _fv_eq_gamma_ti_fv(spinor2, mu, spinor1);
            _co_eq_fv_dag_ti_fv(&w, &g_spinor_field[4*n_c+apsource[nu][ir]*n_c+ia][_GSI(g_iup[ix][mu])], spinor2);
            if(!aisimag[nu]) {
              conn3[_GWI(4*mu+nu,ix,VOLUME)  ] -= assource[nu][ir] * w.re;
              conn3[_GWI(4*mu+nu,ix,VOLUME)+1] -= assource[nu][ir] * w.im;
            } else {
...
*/
  ...
  //write_contraction(conn, NULL, filename, 16, 2, 0);
/*
  sprintf(filename, "aa_x.%.4d", Nconf);
...
*/
  ...
  if(g_cart_id==0) {
    fprintf(stdout, "\n# [mixed_vc_exact2] %s# [mixed_vc_exact2] end of run\n", ctime(&g_the_time));
    fflush(stdout);
    fprintf(stderr, "\n# [mixed_vc_exact2] %s# [mixed_vc_exact2] end of run\n", ctime(&g_the_time));
    fflush(stderr);
  }
